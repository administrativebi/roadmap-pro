...
const MOCK_BADGES = [
    { id: 1, name: "Primeiros Passos", desc: "Completou o primeiro checklist", icon: "🚀", color: "from-blue-400 to-blue-600", unlocked: true, date: "10 Fev 2026" },
    { id: 2, name: "Fogo Diário", desc: "5 dias seguidos", icon: "🔥", color: "from-orange-400 to-red-600", unlocked: true, date: "15 Fev 2026" },
    { id: 3, name: "Visão Laser", desc: "Zero não conformidades em 1 semana", icon: "👁️", color: "from-emerald-400 to-green-600", unlocked: true, date: "20 Fev 2026" },
    { id: 4, name: "Centurião", desc: "100 checklists completados", icon: "💯", color: "from-purple-400 to-indigo-600", unlocked: false },
    { id: 5, name: "Mestre APPCC", desc: "Score 100% em Segurança Alimentar", icon: "🛡️", color: "from-amber-400 to-orange-600", unlocked: false },
];

export default function ProfilePage() {
    const [shieldActive, setShieldActive] = useState(false);
    const [avatarAccessory, setAvatarAccessory] = useState("none"); // none, crown, glasses

    const useShield = () => {
        setShieldActive(true);
        // Simulando uso do escudo e reset na próxima semana
                ...
                <div className="md:col-span-1 bg-white dark:bg-zinc-950 rounded-2xl border border-zinc-100 dark:border-zinc-900 overflow-hidden shadow-sm flex flex-col">
                    <div className="h-32 bg-gradient-to-r from-violet-500 via-purple-500 to-fuchsia-500 relative">
                        <div className="absolute top-4 right-4 bg-black/20 backdrop-blur-md text-white text-xs px-3 py-1 rounded-full font-bold flex items-center gap-1">
                            Nível 3 <Star className="w-3 h-3 fill-white" />
                        </div>
                    </div>
                    <div className="px-6 pb-6 pt-0 flex-1 flex flex-col items-center">
                        ...
                        <h2 className="text-xl font-bold text-zinc-900 dark:text-zinc-50">João Silva</h2>
                        <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-6">Técnico de Qualidade</p>
                        ...
                        <div className="w-full bg-zinc-50 dark:bg-zinc-900 rounded-xl p-4 border border-zinc-100 dark:border-zinc-800">
                            ...
                            <div className="flex justify-center gap-3">
                                <button onClick={() => setAvatarAccessory('none')} className={cn("w-10 h-10 rounded-lg flex items-center justify-center text-xl transition-all", avatarAccessory === 'none' ? "bg-violet-100 dark:bg-violet-900/40 border-2 border-violet-500" : "bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 hover:border-violet-300")}>👤</button>
                                <button onClick={() => setAvatarAccessory('glasses')} className={cn("w-10 h-10 rounded-lg flex items-center justify-center text-xl transition-all", avatarAccessory === 'glasses' ? "bg-violet-100 dark:bg-violet-900/40 border-2 border-violet-500" : "bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 hover:border-violet-300")}>🕶️</button>
                                {/* Crown requires higher level, so maybe a lock overlay or just available for demo */}
                                <button onClick={() => setAvatarAccessory('crown')} className={cn("w-10 h-10 rounded-lg flex items-center justify-center text-xl transition-all", avatarAccessory === 'crown' ? "bg-violet-100 dark:bg-violet-900/40 border-2 border-violet-500" : "bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 hover:border-violet-300")}>👑</button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="md:col-span-2 space-y-6">
                    ...
                    <div className="bg-white dark:bg-zinc-950 rounded-2xl border border-zinc-100 dark:border-zinc-900 p-6 shadow-sm">
                        ...
                        <p className="text-center text-xs text-zinc-500 dark:text-zinc-400 mt-4">
                            Faltam <strong className="text-amber-500 font-bold">450 XP</strong> para alcançar a Liga de Ouro.
                        </p>
                    </div>
                </div>
                    ...
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-zinc-500 dark:text-zinc-400">3 de 15 badges desbloqueadas</p>
                            </div>
                        </div>
                    </div>
